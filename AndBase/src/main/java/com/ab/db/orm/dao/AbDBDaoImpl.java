...
public class AbDBDaoImpl<T> extends AbBasicDBDao implements AbDBDao<T> {
	...
	public AbDBDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		AbLogUtil.d("xxxxxxx    " + AbDBDaoImpl.class, "clazz:" + this.clazz + " tableName:" + this.tableName
	...
	public T queryOne(int id) {
		synchronized (lock) {
			...
			AbLogUtil.d("xxxxxxx    " + AbDBDaoImpl.class, "[queryOne]: select * from " + this.tableName + " where "
					+ this.idColumn + " = '" + id + "'");
	...
	public List<T> rawQuery(String sql, String[] selectionArgs,Class<T> clazz) {
		...
		try {
			lock.lock();
			checkDBOpened();
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[rawQuery]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			getListFromCursor(clazz,list, cursor);
		} catch (Exception e) {
			AbLogUtil.e("xxxxxxx    " +AbDBDaoImpl.class, "[rawQuery] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[isExist]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			if (cursor.getCount() > 0) {
				return true;
			}
		} catch (Exception e) {
			AbLogUtil.e("xxxxxxx    " +AbDBDaoImpl.class, "[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public List<T> queryList(String[] columns, String selection,
			...
			try {
				lock.lock();
				checkDBOpened();
				AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[queryList] from "+this.tableName+" where "+selection+"("+selectionArgs+")"+" group by "+groupBy+" having "+having+" order by "+orderBy+" limit "+limit);
					...
					for(T entity:list){
							...
							if(RelationsType.one2one.equals(type)){
								...
								if(listEntityClazz==null){
									AbLogUtil.e("xxxxxxx    " +AbDBDaoImpl.class, "对象模型需要设置List的泛型");
									return null;
								}
				...
				AbLogUtil.e("xxxxxxx    " +AbDBDaoImpl.class, "[queryList] from DB Exception");
				e.printStackTrace();
			} finally {
				closeCursor(cursor);
				lock.unlock();
	...
	public long insert(T entity) {
		// TODO: 1/24/16
		if (this.tableName.equals("project")){
	...
	public long insert(T entity, boolean flag) {
			...
			try {
				...
				flag = true;
				AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[insert]: insert into " + this.tableName + " " + sql);
				...
				for (Field relationsField : allFields) {
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[insert]: insert into " + relationsTableName + " " + sql);
				...
				AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[insert] into DB Exception.");
				e.printStackTrace();
			}finally {
				lock.unlock();
			}
	...
	public long[] insertList(List<T> entityList,boolean flag) {
			...
			try {
				...
				for(int i=0;i<entityList.size();i++){
					...
					AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[insertList]: insert into " + this.tableName + " " + sql);
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[insertList]: insert into " + relationsTableName + " " + sql);
							db.insert(relationsTableName, null, relationsCv);
						}
				...
				AbLogUtil.d("xxxxxxx    " + AbDBDaoImpl.class, "[insertList] into DB Exception.");
				e.printStackTrace();
			} finally {
				lock.unlock();
			}
	...
	public int delete(int id) {
		...
		try {
			...
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[delete]: delelte from " + this.tableName + " where "
					+ where.replace("?", String.valueOf(id)));
			rows =  db.delete(this.tableName, where, whereValue);
		} catch (Exception e) {
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[delete] DB Exception.");
			e.printStackTrace();
		}finally{
			lock.unlock();
	    }
	...
	public int delete(String whereClause, String[] whereArgs) {
		...
		try {
			...
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[delete]: delete from " + this.tableName + mLogSql);
	...
	public int deleteAll() {
		...
		try {
			lock.lock();
			checkDBOpened();
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[delete]: delete from " + this.tableName );
	...
	public int update(T entity) {
		...
		try {
			...
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[update]: update " + this.tableName + " set " + sql
					+ " where " + where.replace("?", String.valueOf(id)));

			String[] whereValue = { Integer.toString(id) };
			rows = db.update(this.tableName, cv, where, whereValue);
		} catch (Exception e) {
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[update] DB Exception.");
			e.printStackTrace();
		} finally {
			lock.unlock();
		}
	...
	public List<Map<String, String>> queryMapList(String sql,String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[queryMapList]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			while (cursor.moveToNext()) {
			...
			AbLogUtil.e("xxxxxxx    " +AbDBDaoImpl.class, "[queryMapList] from DB exception");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public int queryCount(String sql, String[] selectionArgs) {
       ...
       try{
    	   lock.lock();
    	   checkDBOpened();
    	   AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[queryCount]: " + getLogSql(sql, selectionArgs));
           cursor = db.query(this.tableName, null, sql, selectionArgs, null, null,null);
           if(cursor != null){
        	   count = cursor.getCount();
           }
       }catch (Exception e){
    	   AbLogUtil.e("xxxxxxx    " +AbDBDaoImpl.class, "[queryCount] from DB exception");
           e.printStackTrace();
       }finally{
    	   closeCursor(cursor);
    	   lock.unlock();
	...
	public void execSql(String sql, Object[] selectionArgs) {
		try {
			lock.lock();
			checkDBOpened();
			AbLogUtil.d("xxxxxxx    " +AbDBDaoImpl.class, "[execSql]: " + getLogSql(sql, selectionArgs));
			...
			AbLogUtil.e("xxxxxxx    " +AbDBDaoImpl.class, "[execSql] DB exception.");
			e.printStackTrace();
		} finally {
			lock.unlock();
		}
